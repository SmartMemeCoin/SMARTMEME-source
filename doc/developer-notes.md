...
- [Development guidelines](#development-guidelines)
    - [General Smartmeme Core](#general-smartmeme-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Smartmeme Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/The-Smartmeme-Endeavor/smartmeme/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/smartmemed -printtoconsole
```
...
Smartmeme Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 7282.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 6282 for connections and services them.
...
However, in open source software such as Smartmeme Core, where everyone uses
...
pay attention to for reviewers of Smartmeme Core code.

General Smartmeme Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Smartmeme Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Smartmeme Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `smartmeme-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `smartmeme-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Smartmeme Core, the wallet was always
