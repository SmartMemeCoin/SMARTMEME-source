    ...
    SmartnodeCollaterals collaterals = Params().GetConsensus().nCollaterals;
    {
        ...
        for (auto pcoin : GetSpendableTXs()) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (fGetSMME) {
                    if (fSMMELimitHit) // We hit our limit
                        continue;
                    // We only want SMME OutPoints. Don't include Asset OutPoints
                    if (isAssetScript)
                        continue;
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    /** SMME START */
                    // Check to see if you need to make an asset data outpoint OP_SMME_ASSET data
