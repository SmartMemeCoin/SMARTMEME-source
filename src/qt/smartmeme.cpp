...
{
    return QCoreApplication::translate("smartmeme-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in smartmeme.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in smartmeme.qrc)
...
/** Class encapsulating Smartmeme Core startup and shutdown.
...
/** Main Smartmeme application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // smartmeme: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Smartmeme Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(smartmeme);
    Q_INIT_RESOURCE(smartmeme_locale);
    ...
    /// 6. Determine availability of data directory and parse smartmeme.conf
    ...
    // smartmeme: links repeatedly have their payment requests routed to this process:
