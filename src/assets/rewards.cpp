...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for SMME
    ...
    if (p_rewardSnapshot.strDistributionAsset != "SMME") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is SMME with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using SMME differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "SMME") {
        // Check amount
